...
public class nocows {
	...
	public static int solve(int i, int j) {
		if (i % 2 == 0 || 2 * j - 1 > i)
			return 0;// if out of bounds or # nodes is even
		if (j == 1)
			return i == 1 ? 1 : 0;// if height is 1, only answers are 1 and 0
		if (dp[i][j] != -1)
			return dp[i][j];// if is cached, return cached value
		dp[i][j] = 0;
		for (int a = 1; a < (i + 1) / 2; a += 2) {// loop through possible # of nodes
			...
			for (int b = 1; b + 1 < j; b++) {// try all possible heights from 1 to j-2
				sol += solve(n1, b) * solve(n2, j - 1);// at least 1 must have height j-1 to make parent tree of height
														// j
			...
			sol %= 9901;
			dp[i][j] += ((n1 != n2) ? 2 : 1) * sol;// if # nodes in subtrees are unequal you can double the count by
													// swapping the two subtrees
			dp[i][j] %= 9901;
		}
		return dp[i][j];
	}
